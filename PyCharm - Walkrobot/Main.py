# Daniel Gurtner PyBullet Simulation
...
while (True):
    pos, orn, L, angle, T = pybulletDebug.cam_and_robotstates(boxId)
    # calculates the feet coord for gait, defining length of the step and direction (0ยบ -> forward; 180ยบ -> backward)
    bodytoFeet = trot.loop(L, angle, T, offset, bodytoFeet0)

    #####################################################################################
    #####   kinematics Model: Input body orientation, deviation and foot position    ####
    #####   and get the angles, neccesary to reach that position, for every joint    ####
    FR_angles, FL_angles, BR_angles, BL_angles = robotKinematics.solve(orn, pos, bodytoFeet)
